...
import 'package:movie/models/main_page_data.dart';
import 'package:movie/models/movie.dart';
import 'package:movie/models/search_category.dart';
  ...
  Future<void> getMovies() async {
    try {
      ...
      if (state.searchText!.isEmpty) {
        if (state.serachCategory == SearchCategory.popular) {
          _movies = await (_movieService.getPopularMovies(page: state.page));
        } else if (state.serachCategory == SearchCategory.upcoming) {
          _movies = await (_movieService.getUpcomingMovies(page: state.page));
        } else if (state.serachCategory == SearchCategory.none) {
          _movies = [];
        }
      } else {
